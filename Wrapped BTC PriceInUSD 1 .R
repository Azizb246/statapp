...
merged_df <- df_price %>%
  ...
  inner_join(df_reserve %>% rename(datetime = regular_datetime) %>%
               dplyr::select(datetime, totalATokenSupply, 
                             totalCurrentVariableDebt, totalLiquidity, availableLiquidity , utilizationRate ), by = "datetime")
...
plot_list <- list(
  ...
  ggplot(merged_df, aes(x = datetime, y = totalATokenSupply)) + 
    geom_line(color = "blue") + labs(title = "Total AToken Supply Evolution", x = "Time", y = "Rate") + theme_minimal(),
  
  ggplot(merged_df, aes(x = datetime, y = totalCurrentVariableDebt)) + 
    geom_line(color = "red") + labs(title = "Total Current Variable Debt Evolution", x = "Time", y = "Rate") + theme_minimal(),
  
  ggplot(merged_df, aes(x = datetime, y = totalLiquidity)) + 
    geom_line(color = "green") + labs(title = "Total Liquidity Evolution", x = "Time", y = "Rate") + theme_minimal(),
  
  ggplot(merged_df, aes(x = datetime, y = availableLiquidity)) + 
    geom_line(color = "purple") + labs(title = "Available Liquidity Evolution", x = "Time", y = "Debt") + theme_minimal(),
...
variables <- c("pct_change_totalATokenSupply", "pct_change_totalCurrentVariableDebt",
               "pct_change_totalLiquidity", "pct_change_availableLiquidity", "pct_change_utilizationRate")
...
lag_range <- 1:50  # Ajustable selon l'analyse
